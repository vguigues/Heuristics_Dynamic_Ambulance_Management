    ...
    for j=1:nb_ambulances
        ...
        if (bool)
            ...
            if (ambulances(j).arrival_time_at_b_last_trip<=calls(i).time)
                if (traveltype=='G')
                    time_this_ambulance=travel_time_geodesic(ambulances(j).base.lat,ambulances(j).base.long,
                    calls(i).lat,calls(i).long,ambulances(j).speed,'sphere',Rearth);
                else
                    [time_this_ambulance,L]=street_travel_time(ambulances(j).base,calls(i).id,ambulances(j).speed,graph);             
                end
            ...
            [penalized_response]=penalized_response_time(penalization,time_this_ambulance,penalty_matrix,samu_type,ambulances(j).type,calls(i).priority,quality_coeff);
            if ((penalized_response<mintimep)||((penalized_response==mintimep)&&(ambulances(j).type>ambulances(index_amb).type)))
                index_amb=j;
                mintime=time_this_ambulance;
                mintimep=penalized_response;
            end
        end
    end
