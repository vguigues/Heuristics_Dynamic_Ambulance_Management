...
while (calls_attended<nb_calls)
    ...
    if length(queue_size>=2)
        ...
        %for call with index queue(k) this time is
        %priority_factor*(current_time-timecall)
        %where timecall=calls(queue(k)).time, priority_factor=penalization(calls(queue(k)).priority)
    ...
    for k=1:queue_size
        ...
        for j=1:nb_ambulances
                if (ambulances(j).type<=calls(queue(k)).priority)
                    ...
                    elseif (time_this_ambulance<mintimek)
                        index_ambs=[j];
        ...
        if (index_amb>0)
            if (ambulances(index_amb).arrival_time_at_b_last_trip<=current_time)
                ...
                if  (ambulances(index_amb).arrival_time_at_f_last_trip==current_time)
                    ...
                    len=length(ambulances_times{1,index_amb});
                    [curr_lat,curr_long]=position_between_origin_destination(ambulances(index_amb).hospital_destination.lat,ambulances(index_amb).hospital_destination.long,ambulances(index_amb).base.lat,ambulances(index_amb).base.long,ambulances(index_amb).arrival_time_at_f_last_trip,current_time,ambulances(index_amb).speed);
