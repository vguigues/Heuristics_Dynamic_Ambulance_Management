...
function [waiting_on_scene,waiting_to_hospital,waiting_on_scene_penalized,waiting_to_hospital_penalized,
        ambulances_times,ambulances_trips,trip_type,calls_end] = closest_among_all_forward_nomyopic(calls,ambulances,
        nb_ambulances,hospitals,nb_hospitals,nb_calls,penalization,ambulances_times,ambulances_trips,trip_type,bases,
        nb_base,cleaning_bases)
            ...
            for j=1:nb_ambulances
                ...
                if (ambulances(j).type<=calls(i).priority)
                    if (ambulances(j).arrival_time_at_b_last_trip<=calls(i).time)
                        travelTimes(i,j)=travel_time(ambulances(j).base.lat,ambulances(j).base.long,calls(i).lat,
                            calls(i).long,ambulances(j).speed);
                    elseif (ambulances(j).arrival_time_at_f_last_trip<=calls(i).time)
                        [curr_lat,curr_long]=position_between_origin_destination(ambulances(j).free_destination.lat,
                            ambulances(j).free_destination.long,ambulances(j).base.lat,ambulances(j).base.long,
                            ambulances(j).arrival_time_at_f_last_trip,calls(i).time,ambulances(j).speed);
                        ...
                        time_from_hosp_to_call=travel_time(ambulances(j).free_destination.lat,
                            ambulances(j).free_destination.long,calls(i).lat,calls(i).long,ambulances(j).speed);
                        travelTimes(i,j)=time_to_hospital+time_from_hosp_to_call;
                    end
                    if (travelTimes(i,j)<mintime(i))
                        index_ambs{1,i}=[j];
                        mintime(i)=travelTimes(i,j);
                        mintimep(i)=penalization(calls(i).priority)*mintime(i);
                    elseif (travelTimes(i,j)==mintime(i))
        ...
        while (isAllocated(i)==0)
            ...
            if (index>-1)
                ...
                [ambulances,waiting_on_scene,waiting_to_hospital,waiting_on_scene_penalized,waiting_to_hospital_penalized,
                    ambulances_times,ambulances_trips,trip_type,calls_end] = allocate_ambulance(index_amb(i),i,ambulances,
                    calls,trip_type,ambulances_times,ambulances_trips,mintime(i),hospitals,penalization,cleaning_bases,
                    waiting_on_scene,waiting_to_hospital,calls_end,waiting_on_scene_penalized,waiting_to_hospital_penalized);
                    ...
                    if (isAllocated(k)==0)
                        if (ambulances(index_amb(i)).type<=calls(k).priority)
                            if (ambulances(index_amb(i)).arrival_time_at_b_last_trip<=calls(k).time)
                                travelTimes(k,index_amb(i))=travel_time(ambulances(index_amb(i)).base.lat,
                                    ambulances(index_amb(i)).base.long,calls(k).lat,calls(k).long,
                                    ambulances(index_amb(i)).speed);
                            elseif (ambulances(index_amb(i)).arrival_time_at_f_last_trip<=calls(k).time)
                                [curr_lat,curr_long]=position_between_origin_destination(
                                    ambulances(index_amb(i)).free_destination.lat,
                                    ambulances(index_amb(i)).free_destination.long,ambulances(index_amb(i)).base.lat,
                                    ambulances(index_amb(i)).base.long,
                                    ambulances(index_amb(i)).arrival_time_at_f_last_trip,calls(k).time,
                                    ambulances(index_amb(i)).speed);
                                travelTimes(k,index_amb(i))=travel_time(curr_lat,curr_long,calls(k).lat,calls(k).long,
                                    ambulances(index_amb(i)).speed);
                            else
                                time_to_hospital=ambulances(index_amb(i)).arrival_time_at_f_last_trip-calls(k).time;
                                time_from_hosp_to_call=travel_time(ambulances(index_amb(i)).free_destination.lat,
                                    ambulances(index_amb(i)).free_destination.long,calls(k).lat,calls(k).long,
                                    ambulances(index_amb(i)).speed);
                            ...
                            for j=1:nb_ambulances
                                if (ambulances(j).type<=calls(k).priority)
                                    if (travelTimes(k,j)<mintime(k))
                                        ...
                                        mintimep(k)=penalization(calls(k).priority)*mintime(k);
                ...
                while ((isAllocated(i)==0)&&(ind<=indL))
                    [whichk,maxIndex,travelTimes,index_ambs]=foundambulance(index_ambs,calls,maxIndex,i,ambulances,
                        index_ambs{1,i}(ind),isAllocated,nb_calls,nb_ambulances,mintime,penalization);
                    if (whichk>0)
                        isAllocated(whichk)=1;
                        [ambulances,waiting_on_scene,waiting_to_hospital,waiting_on_scene_penalized,
                            waiting_to_hospital_penalized,ambulances_times,ambulances_trips,trip_type,calls_end] = 
                                allocate_ambulance(index_ambs{1,i}(ind),whichk,ambulances,calls,trip_type,
                                    ambulances_times,ambulances_trips,mintime(whichk),hospitals,penalization,
                                    cleaning_bases,waiting_on_scene,waiting_to_hospital,calls_end,
                                    waiting_on_scene_penalized,waiting_to_hospital_penalized);
