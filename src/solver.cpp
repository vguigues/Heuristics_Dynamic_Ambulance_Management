...
double Solver::penalized_response_time(double response_time, int amb_type,
  ...
  if (g_params.amb_setup == "us") {
    ...
    return response_time * ins.penalties[call_type] +
           1500 * ins.penalty_matrix[amb_type][call_type];
...
int Solver::find_best_base(int amb_id, bool debug) {
  ...
  for (int c = amb.type; c < ins.nb_priorities; ++c) {
    vector<pair<int, int>> index_bases;
    for (size_t b = 0; b < ins.bases.size(); ++b) {
      ...
      double lambda = 0;
      if (index >= times.size()) {  // if arrival after the end of window, get
        ...
        if (index < times.size()) {
          ...
          lambda +=
              (total_time - times.back()) * lambda_base[b][times.size() - 1][c];
        }
      }
      ...
      if (debug) {
        fmt::print(
            "c = {}, total_th = {}, index final = {}, lambda = {}, "
            "nb_calls = {}, nb_amb = {}\n",
            c, (time_arrival + time_ahead) / 3600, index + 1, lambda, nb_calls,
            nb_amb);
      }
      double deficit_c = nb_calls - nb_amb;
      index_bases.push_back(make_pair(deficit_c, b));
    }
    sort(index_bases.begin(), index_bases.end(),
         std::greater<pair<int, int>>());
    int deficit_max = index_bases[0].first;
    if (deficit_max > 0) {
      double min_time = GRB_INFINITY;
      int min_index = -1;
      size_t k = 0;
      while (k < index_bases.size() && index_bases[k].first == deficit_max) {
        auto base = ins.bases[index_bases[k].second];
      ...
      return index_bases[min_index].second;
    }
  }
  return -1;
}
    ...
    for (size_t i = 0; i < queue.size(); ++i) {
      ...
      for (auto &amb : ambulances) {
        ...
        if (can_answer(amb, call)) {
          ...
          // 	fmt::print("\tamb {}, type {}, time {:.1f}, pen_time {:.1f},
          // busy {}\n",amb.id, amb.type, travel_times[i][j],
      ...
      if (index_amb >= 0) {
        ...
        for (auto ind : remaining_indexes) {
          ...
          if (ind != current_call && has_index_amb) {
            ...
            if (debug) {
              fmt::print("\tCall {}: pen = {:.1f}, index_amb = {}\n",
  ...
  while (static_cast<size_t>(calls_attended) < calls.size()) {
    ...
    if (event_call == 1) {
      ...
      for (auto &amb : ambulances) {
        ...
        if (amb.arrival_time_at_f_last_trip <= time &&
            pen_resp_time < min_time && can_answer(amb, call)) {
          min_time = pen_resp_time;
          index_amb = amb.id;
        }
      }
      if (debug) {
  ...
  while (static_cast<size_t>(calls_attended) < calls.size()) {
    ...
    if (event_call == 1) {
      ...
      for (Ambulance &amb : ambulances) {
        ...
        if (can_answer(amb, call) && pen_response_time < min_time) {
          min_time = pen_response_time;
        }
      }
...
DistrictData DistrictManager::get_m_districts() {
  ...
  vector<int> ambulance_district(ins.nb_bases, -1);
  for (int r = 0; r < ins.nb_regions; ++r) {
    poisson_distribution<int> dist(lambda[r]);
...
void MCSolver::read_preparedness() {
  preparedness = ins.preparedness;
  // ulong nb, nt, nta,n0,n1,n2;
  // string file_name = fmt::format("preps/preparedness_{}a_{}b.txt",
  // ins.nb_ambulances, ins.nb_bases);
  // // fmt::print("file_name = {}\n",file_name);
  // ifstream prep_file(file_name, std::ios::in);

  // prep_file >> nb >> nt >> nta;
  // if(nb != static_cast<ulong>(ins.nb_bases)){
  // 	fmt::print("Error: number of bases in preparedness file {} not
  // compatible with number of bases in the simulator: {}", nb, ins.nb_bases);
  // 	exit(1);
  // }
  // if(nt != ins.time_horizon.size()){
  // 	fmt::print("Error: time horizon size in preparedness file {} not
  // compatible with time horizon size in the simulator: {}", nt,
  // ins.time_horizon.size()); 	exit(1);
  // }

  // if(nta != static_cast<ulong>(ins.nb_types_ambulance)){
  // 	fmt::print("Error: preparedness file has {} types of ambulance but
  // instance has {}.\n", nta,ins.nb_types_ambulance); 	exit(1);
  // }

  // if(nta == 2){
  // 	prep_file >> n0 >> n1;
  // 	if(n0 != (ulong) ins.amb_count_by_type[0] || n1 != (ulong)
  // ins.amb_count_by_type[1]){ 		fmt::print("Error: Supply in
  // preparedness file
  // ({}, {}) not compatible with supply of simulator ({}, {})\n",
  // 			n0,n1,ins.amb_count_by_type[0],
  // ins.amb_count_by_type[1]); 			exit(1);
  // 	}
  ...
  // 	prep_file >> n0 >> n1 >> n2;
  // 	if(n0 != (ulong) ins.amb_count_by_type[0] || n1 != (ulong)
  // ins.amb_count_by_type[1] || 		n2 != (ulong)
  // ins.amb_count_by_type[2]){ 		fmt::print("Error: Supply in
  // preparedness file
  // ({}, {}, {}) not compatible with supply of simulator ({}, {}, {})\n",
  // n0,n1,n2, ins.amb_count_by_type[0], ins.amb_count_by_type[1],
  // ins.amb_count_by_type[2]); 			exit(1);
  // 	}
...
bool MCSolver::solve_selection(int i0, bool debug) {
  ...
  if (index_amb != -1) {
    ...
    if (debug) {
      fmt::print(
          "Call {} answered by {}, finish_service {:.1f}, Real_min_time "
          "= {:.1f}\n",
          call.id, amb.id, amb.arrival_time_at_f_last_trip, real_min_time);
    }
    if (!is_new_call) {
...
bool MCSolver::solve_reassignment(int a0, bool debug) {
        ...
        if (can_answer(amb, calls[queue[i]])) {
          ...
          if (debug) {
            fmt::print("coeff - {} = {}, GAMMA = {}, sm = {}\n",
                       x[a][i].get(GRB_StringAttr_VarName), GAMMA * sm, GAMMA,
                       sm);
          }
          fo += GAMMA * s_minus(amb.type, b, time, A_b[b]) * x[a][i];
        }
    ...
    for (size_t b = 0; b < ins.bases.size(); ++b) {
      ...
      if (debug) {
        fmt::print("coeff + {} = {}, GAMMA = {}, sp = {}\n",
                   y[a][b].get(GRB_StringAttr_VarName), GAMMA * sp, GAMMA, sp);
      }
      fo += GAMMA * sp * y[a][b];
    }
  }
  ...
  if (call_index != -1) {
    ...
    if (debug) {
      fmt::print(
          "Call {} answered by {}, finish_service {:.1f}. "
          "Real_mean_time = {:.1f}\n",
          call.id, amb.id, amb.arrival_time_at_f_last_trip, real_min_time);
    }
    return true;
...
void MCSolver::run() {
  ...
  while (static_cast<size_t>(calls_attended) < calls.size()) {
    ...
    if (debug) {
      fmt::print(
          "MarkovSolver: Time {:.1f}, index = {}, event = {}, queue = "
          "{}, att = {}/{}\n",
          time, index_call, event_call, queue, calls_attended, calls.size());
    }
