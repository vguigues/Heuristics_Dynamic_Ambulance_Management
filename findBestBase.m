...
%The ambulance is at (graph(0l0).lat, graph(l0).long) at time t0
...
function [indexBase]=findBestBase(ambulances,graph,t0,l0,lambdaBase,T,nb_base,bases,
    indexAmb,timeAhead,times,nbpriorities,nb_ambulances,poissquant)

times=[times,times+times(T)];

for ibase=1:nb_base
    [t,listnodes]=street_travel_time(l0,bases(ibase).id,ambulances(indexAmb).speed,graph);
...
while ((thispriority<=3)&&(contd))
    ...
    for ibase=1:nb_base
        ...
        lambda=(times(index)-timeArrival(ibase))*lambdaBase(ibase,index,thispriority);
        while (times(index+1)<=timeArrival(ibase)+timeAhead)
            lambda=lambda+(times(index+1)-times(index))*lambdaBase(ibase,index+1,thispriority);
            index=index+1;
        end
        lambda=(timeArrival(ibase)+timeAhead-times(index))*lambdaBase(ibase,index+1,thispriority);
